...
<script type="text/javascript">
	function checkMember() {
		let regExpId = /^[a-z|A-Z|ㄱ-ㅎ|ㅏ-ㅣ|가-힣]/; // 아이디는 영문 대/소문자, 한글, 한글의 자음과 모음으로 시작
		...
		let regExpEmail = /^[0-9a-z-A-Z]([-_\.]?[0-9a-zA-Z])*@[0-9a-zA-Z]([-_\.]?[0-9a-zA-Z])*\.[a-zA-Z]{2,3}$/i; // 이메일 형태만 가능  i:ignore(대소문자 구분 안 함)
		...
		if(!regExpName.test(name)){
			alert("이름은 한글로 입력해야 합니다."); // smith(x), 스미스(o)
			form.name.select();
			return;
		}
...
<body>
	...
	<form name="Member" method="post" action="validation05_process.jsp">
		...
		<p><input type="button" value="가입쓰" onclick="checkMember()" /></p>
	</form>
</body>
</html>