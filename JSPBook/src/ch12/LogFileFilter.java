...
public class LogFileFilter implements Filter {
	...
	public void init(FilterConfig filterConfig) throws ServletException {
		...
		// 파일이 있으면..
		try {
			// 1) PrintWriter: byte출력 스트림과 문자 출력 스트림을 가지고 객체를 생성할 수 있음. 자동 flush 기능 탑재
			//					생성자에 FileNotFoundException 예외가 있어서 반드시 예외 처리를 해야 함!
			//					new PrintWriter(Writer out): 자동 플러시 없음
			//					new PrintWriter(Writer out, true): 자동 플러시 있음
			// 2) FileWriter: 텍스트 데이터를 파일에 저장할 때 사용. 문자 단위로 저장하므로 텍스트만 저장 가능
